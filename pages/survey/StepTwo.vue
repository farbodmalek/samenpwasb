...
const NextstepHandel = () => {
  ...
  if (!v$.value.$invalid) {
    ...
    if (form.numberOfInsurdPerson > form.numberOfJobsCreated) {
      ToastNotificationService.warn("تعداد بیمه شده نمیتواند بیشتر از اشتغال ایجاد شده باشد");
      ...
      router.push({ path: "/survey/StepThree", query: {loanId:data.loanId ,loanType:loanSurvey }});
      localStorage.setItem("SecPreForm", JSON.stringify(form));
    }
  } else {
